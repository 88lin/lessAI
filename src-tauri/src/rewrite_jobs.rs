...
fn clamp_chunks_per_request(value: usize) -> usize {
    value.max(1)
}

fn snapshot_running_indices_from_batches(in_flight_batches: &[Vec<usize>]) -> Vec<usize> {
...
pub(crate) async fn run_manual_rewrite(
    ...
    let batch_size = clamp_chunks_per_request(settings.chunks_per_request);
    let next_batch =
        rewrite_targets::find_next_manual_batch(&session.chunks, target_indices.as_ref(), batch_size);
    ...
    process_chunk_batch(app, state, &session.id, &next_batch, false).await?;
...
async fn run_auto_loop(
    ...
    let chunks_per_request = clamp_chunks_per_request(settings.chunks_per_request);
    ...
    let mut tasks: tokio::task::JoinSet<(Vec<usize>, Result<Vec<String>, String>)> =
        tokio::task::JoinSet::new();
    let mut in_flight_batches = Vec::<Vec<usize>>::new();
    ...
    loop {
        ...
        while !job.paused.load(Ordering::SeqCst) && in_flight_batches.len() < max_concurrency {
            let batch_indices =
                rewrite_targets::take_next_auto_batch(&mut pending, chunks_per_request);
            ...
            mark_chunks_running(&app, app_state, &session_id, &batch_indices)?;
            in_flight_batches.push(batch_indices.clone());

            let batch_source_texts = batch_indices
                .iter()
                .map(|index| {
                    source_texts
                        .get(*index)
        ...
        if pending.is_empty() && in_flight_batches.is_empty() {
            break;
        }
        ...
        match tokio::time::timeout(std::time::Duration::from_millis(250), tasks.join_next()).await {
            Ok(Some(joined)) => match joined {
                Ok((indices, Ok(candidate_texts))) => {
                    remove_in_flight_batch(&mut in_flight_batches, &indices);
                    let completed_batch = match commit_chunk_batch_success(
                        &app,
                        app_state,
                        &session_id,
                        &indices,
                        ...
                        Err(error) => {
                            tasks.abort_all();
                            commit_chunks_failure(&app, app_state, &session_id, &indices, error.clone())?;
                            reset_running_chunks_to_idle(&app, app_state, &session_id)?;
                            return Err(error);
                        }
                    };
                    completed_chunks = completed_chunks.saturating_add(completed_batch.len());
                    for (index, suggestion_id, suggestion_sequence) in completed_batch {
                        app.emit(
                            "chunk_completed",
                            ChunkCompletedEvent {
                                session_id: session_id.clone(),
...
pub(crate) async fn process_chunk_batch(
    app: &AppHandle,
    state: &AppState,
    session_id: &str,
    indices: &[usize],
    ...
    let source_texts = indices
        ...
        .collect::<Result<Vec<_>, _>>()?;
    mark_chunks_running(app, state, session_id, indices)?;

    match rewrite::rewrite_chunks(&settings, &source_texts, format).await {
        Ok(candidate_texts) => {
            ...
            let set_status = (!auto_approve).then_some(RunningState::Idle);
            let completed_batch = match commit_chunk_batch_success(
                app,
                state,
                session_id,
...
fn commit_chunk_batch_success(
    ...
    with_session_lock(state, session_id, || {
        ...
        for (index, candidate_text) in indices.iter().copied().zip(candidate_texts.into_iter()) {
            ...
            normalized.insert(
                index,
                normalize_candidate_text(&chunk_source_text, &candidate_text, line_ending),
            );
        }

        validate_candidate_batch_writeback(&latest, &normalized)?;
        ...
        for index in indices.iter().copied() {
            ...
            chunk.error_message = None;
            committed.push((index, suggestion_id, suggestion_sequence));
        }
...
fn validate_candidate_batch_writeback(
    session: &DocumentSession,
    overrides: &HashMap<usize, String>,
) -> Result<(), String> {
    let mut preview = session.clone();
    for (index, candidate_text) in overrides.iter() {
...
fn normalize_candidate_text(
    ...
    candidate_text: &str,
    line_ending: rewrite::LineEnding,
) -> String {
    let mut normalized = candidate_text.to_string();
...
fn remove_in_flight_batch(in_flight_batches: &mut Vec<Vec<usize>>, indices: &[usize]) {
    if let Some(position) = in_flight_batches.iter().position(|batch| batch == indices) {
